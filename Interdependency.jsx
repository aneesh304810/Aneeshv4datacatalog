...
// ---- Swimlane ----
function Swimlane({ t, graph, domains, domFilter, sel, setSel, nodeById, collapsed, toggleLane }) {
  ...
  graph.edges.forEach((e, i) => {
    ...
    paths.push(
      <path key={`p${i}`} d={`M${x1},${y1} C${mx},${y1} ${mx},${y2} ${x2},${y2}`}
        ...
        opacity={dim ? 0.08 : (cross ? 0.8 : 0.5)} strokeDasharray={mismatch ? "5,4" : undefined}
    ...
    if (hlEdge || (cross && hlNode)) {
      labels.push(
        <g key={`l${i}`} style={{ cursor: "pointer" }} onClick={() => setSel({ type: "edge", i })}>
          <rect x={mx - 32} y={(y1 + y2) / 2 - 8} width={64} height={15} rx={7} fill="#fff"
