...
// ---- Swimlane ----
function Swimlane({ t, graph, domains, domFilter, sel, setSel, nodeById }) {
  const W = 1160, laneH = 72;
  const posY = {}; domains.forEach((d, i) => { posY[d] = 16 + i * laneH; });
  // assign a tier by connectivity: nodes that are referenced by many => left
  const inDeg = {}; graph.nodes.forEach((n) => { inDeg[n.id] = 0; });
  graph.edges.forEach((e) => { inDeg[e.a] = (inDeg[e.a] || 0) + 1; inDeg[e.b] = (inDeg[e.b] || 0) + 1; });
  // simple tiering: bucket by degree into 4 columns
  ...
  graph.nodes.forEach((n) => {
    const key = `${n.domain}:${tierOf[n.id]}`;
    perLaneTier[key] = (perLaneTier[key] || 0);
    const yOff = perLaneTier[key] * (NH + 6);
    perLaneTier[key] += 1;
    pos[n.id] = { x: tierX[tierOf[n.id]], y: posY[n.domain] + 6 + yOff };
  });
  const height = 16 + domains.length * Math.max(laneH, 1) + 40;

  const paths = []; const labels = []; const nodes = [];
  graph.edges.forEach((e, i) => {
    const A = pos[e.a], B = pos[e.b], na = nodeById(e.a), nb = nodeById(e.b);
    if (!A || !B || !na || !nb) return;
    ...
    paths.push(
      <path key={`p${i}`} d={`M${x1},${y1} C${mx},${y1} ${mx},${y2} ${x2},${y2}`}
        stroke={col} strokeWidth={hlEdge || hlNode ? 2.5 : (cross ? 1.7 : 1.3)} fill="none"
        opacity={dim ? 0.1 : (cross ? 0.85 : 0.6)} strokeDasharray={mismatch ? "5,4" : undefined}
        style={{ cursor: "pointer" }} onClick={() => setSel({ type: "edge", i })} />
    );
    if (cross || hlEdge) {
      labels.push(
        <g key={`l${i}`} style={{ cursor: "pointer" }} opacity={dim ? 0.15 : 1} onClick={() => setSel({ type: "edge", i })}>
          <rect x={mx - 30} y={(y1 + y2) / 2 - 8} width={60} height={15} rx={7} fill="#fff"
            stroke={hlEdge ? "#0091bf" : (mismatch ? "#ca8a04" : "#f0d0d5")} />
          <text x={mx} y={(y1 + y2) / 2 + 3} fontSize={8} fontFamily="monospace"
  ...
  graph.nodes.forEach((n) => {
    ...
    nodes.push(
      <g key={n.id} style={{ cursor: "pointer" }} opacity={dim ? 0.3 : 1} onClick={() => setSel({ type: "feed", id: n.id })}>
        ...
        <text x={p.x + 14} y={p.y + 18} fontSize={11.5} fontWeight={600} fill="#10193b">
          {n.name.length > 19 ? n.name.slice(0, 18) + "\u2026" : n.name}</text>
        <text x={p.x + 14} y={p.y + 31} fontSize={8.5} fill="#8a97a3">{n.domain}</text>
      </g>
    );
  });
  ...
  return (
    <svg width={W} height={height} style={{ display: "block" }}>
      {domains.map((d, i) => {
        const y = posY[d]; const dim = domFilter && domFilter !== d;
        return (
          <g key={d} opacity={dim ? 0.5 : 1}>
            <rect x={0} y={y} width={W} height={laneH - 6} fill={i % 2 ? "#f7fafb" : "#ffffff"} />
            <rect x={0} y={y} width={5} height={laneH - 6} fill={domColor(domains, d)} />
            <text x={13} y={y + 16} fontSize={10} fontWeight={700} fill={domColor(domains, d)}>{d.toUpperCase()}</text>
          </g>
        );
      })}
      {paths}{labels}{nodes}
