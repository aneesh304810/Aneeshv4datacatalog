...
UPDATE datasets d
SET d.domain = CASE
    WHEN LOWER(d.object_name) LIKE '%account%'      THEN 'Account & Client'
    WHEN LOWER(d.object_name) LIKE '%client%'       THEN 'Account & Client'
    ...
    WHEN LOWER(d.object_name) LIKE '%fee%'          THEN 'Fee & Billing'
    WHEN LOWER(d.object_name) LIKE '%accrual%'      THEN 'Fee & Billing'
    WHEN LOWER(d.object_name) LIKE '%bill%'         THEN 'Fee & Billing'
    WHEN LOWER(d.object_name) LIKE '%asset%'        THEN 'Reference & Asset'
    WHEN LOWER(d.object_name) LIKE '%security%'     THEN 'Reference & Asset'
    WHEN LOWER(d.object_name) LIKE '%instrument%'   THEN 'Reference & Asset'
    WHEN LOWER(d.object_name) LIKE '%portfolio%'    THEN 'Portfolio & Model'
    WHEN LOWER(d.object_name) LIKE '%model%'        THEN 'Portfolio & Model'
    WHEN LOWER(d.object_name) LIKE '%allocation%'   THEN 'Portfolio & Model'
    WHEN LOWER(d.object_name) LIKE '%fx%'           THEN 'FX & Derivatives'
    WHEN LOWER(d.object_name) LIKE '%forward%'      THEN 'FX & Derivatives'
...
SET l.business_domain = CASE
    WHEN LOWER(l.loader_name) LIKE '%account%'     THEN 'Account & Client'
    WHEN LOWER(l.loader_name) LIKE '%client%'      THEN 'Account & Client'
    ...
    WHEN LOWER(l.loader_name) LIKE '%fee%'         THEN 'Fee & Billing'
    WHEN LOWER(l.loader_name) LIKE '%asset%'       THEN 'Reference & Asset'
    WHEN LOWER(l.loader_name) LIKE '%portfolio%'   THEN 'Portfolio & Model'
    WHEN LOWER(l.loader_name) LIKE '%model%'       THEN 'Portfolio & Model'
    ELSE 'Other'
  END
